...
angular.module('angular-growl').directive('growl', [function () {
    ...
    return {
        ...
        templateUrl: function (element, attr) {
            var templateUrl = attr.templateUrl || 'templates/growl/growl.html';
            return templateUrl;
        },
        replace: false,
        scope: {
        ...
        controller: [
          ...
          function ($scope, $timeout, growl, growlMessages) {
              ...
              $scope.position = $scope.position || growl.position(); //LIAM 24/08/2017: Added position
...
angular.module('angular-growl').run([
  ...
  function ($templateCache) {
      ...
      if ($templateCache.get('templates/growl/growl.html') === undefined) {
          $templateCache.put('templates/growl/growl.html', '<div class="growl-container" ng-class="wrapperClasses()">' + '<div class="growl-item alert {{message.isAnimate ? \'am-slide-right\' : \'\'}}" ng-repeat="message in growlMessages.directives[referenceId].messages" ng-class="alertClasses(message)" ng-click="stopTimeoutClose(message)">' + '<button type="button" class="close" data-dismiss="alert" aria-hidden="true" ng-click="growlMessages.deleteMessage(message)" ng-show="!message.disableCloseButton">&times;</button>' + '<button type="button" class="close" aria-hidden="true" ng-show="showCountDown(message)">{{message.countdown}}</button>' + '<h4 class="growl-title" ng-show="message.title" ng-bind="message.title"></h4>' + '<div class="growl-message" ng-bind-html="message.text"></div>' + '</div>' + '</div>');
      }
  }
]);
      ...
      function ($rootScope, $interpolate, $sce, $filter, $timeout, growlMessages) {
          ...
          function sendMessage(text, config, severity) {
              ...
              message = {
                  ...
                  title: _config.title,
                  severity: severity,
                  isAnimate: _config.isAnimate,
                  ttl: _config.ttl || _ttl[severity],
                  timestamp: Date.now(),    //KHA 28/08/2017: Added Custom timestamp config
                  showTimestamp: _config.showTimestamp === undefined ? false : _config.showTimestamp,     //KHA 28/08/2017: Added Custom timestamp config, default is false
          ...
          function clearMessage(referenceId) {      //KHA 03/12/2014: Added Custom clear messages
              growlMessages.clearMessage(referenceId || _referenceId);
          }
          return {
              warning: warning,
              error: error,
              ...
              clearMessage: clearMessage,      //KHA 03/12/2014: Added Custom clear messages
              position: position
          };
      }
    ];
...
angular.module('angular-growl').service('growlMessages', [
  ...
  function ($sce, $timeout) {
      ...
      this.clearMessage = function (referenceId) {      //KHA 03/12/2014: Added Custom clear messages
          var messages = this.directives[referenceId].messages;
